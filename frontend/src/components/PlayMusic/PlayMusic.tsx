...
import { MusicStore } from '../../redux/selectors';
import { getAllMusic } from '../../redux/slices/MusicSlice';
    ...
    useEffect(()=>{
        dispatch(getAllMusic())
            .then((res:any)=>{
                ...
                {
                    setCurMusic(res.payload.data[4].music_link)
                    setMusicTitle(res.payload.data[4].title)
                }
            })
    },[])
    console.log(musicTitle, curMusic)
    return (<>
        <div className={styles.box}>
            <audio ref={audioEl} className={styles.audio} controls loop >
                <source src={curMusic} type="audio/mpeg"></source>
            </audio>
        ...
        >
            ...
            <Modal.Body>
                Current song:{" "}
                <span className={styles.currMusic}>{musicTitle}</span>
                <div className={styles.list}>
                    ...
                    {
                        music.data &&
                        music.data.map((item: any) => {
                            return (
                                <div className={styles.item} onClick={() => {
                                    setCurMusic(item.music_link)
