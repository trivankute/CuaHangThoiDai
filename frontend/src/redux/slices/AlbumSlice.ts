...
const AlbumSlice = createSlice({
    ...
    extraReducers(builder) {
        builder
            ...
            .addCase(updateAlbum.pending, (state, action) => {
                state.loading = true
            })
...
export const deleteAlbum = createAsyncThunk('deleteAlbum', async (input: any) => {
    const { id } = input
    console.log(id)
    try {
        const { data } = await axios.get(`${serverUrl}/api/albums/delete.php/?id=${id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
            }
        });
